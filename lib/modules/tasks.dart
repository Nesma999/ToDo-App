  ...
  Widget build(BuildContext context) {
    return BlocConsumer<BottomNavigationCubit, AppStates>(
        ...
        builder: (context, state) {
          var tasks = BottomNavigationCubit.get(context).Newtasks.length;
          return tasks > 0
              ? ListView.separated(
                  itemBuilder: (context, index) {
                    return buildTaskItem(
                        BottomNavigationCubit.get(context).Newtasks[index],
                        context);
                  },
                  ...
                  itemCount: BottomNavigationCubit.get(context).Newtasks.length,
                )
              : Center(
                  child: Column(
